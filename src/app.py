        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            if not obs_data:
                ...
                if obs_data.get("status") == "SUCCESS":
                    if "data" in obs_data:
                        d = obs_data["data"]
                        if "order_id" in d or "tracking_id" in d:
                            final_answer = (
                                f"Thông tin đơn hàng {d.get('order_id', '')} (Mã vận đơn: {d.get('tracking_id', '')}): "
                                f"Sản phẩm: {d.get('name', '')}, Giá: {d.get('price', 0):,} VNĐ, "
                                f"Số lượng: {d.get('quantity', '')}, Trạng thái: {d.get('status', '')}, "
                                f"Địa điểm: {d.get('place', '')}, Thời gian: {d.get('datetime', '')}."
                            )
                        elif "full_name" in d or "student_id" in obs_data:
                            final_answer = (
                            ...
                            final_answer = f"Kết quả tra cứu thành công: {json.dumps(d, ensure_ascii=False)}"
...
if __name__ == "__main__":
    ...
    print("📦 TRỢ LÝ ĐƠN HÀNG & KHO VẬN (SUPPLY CHAIN AGENT)")
    print("==========================================================")
    ...
    if "--interactive" in sys.argv:
        ...
        print("   - Tra cứu vận đơn: 'Hãy tra cứu thông tin vận đơn có mã Order #1'")
        print("   - Tra cứu kho hàng/vị trí: 'Cho tôi biết địa chỉ kho hàng của đơn hàng Order #1'")
        print("   - Tra cứu trạng thái: 'Tra cứu trạng thái đơn hàng có mã Order #2'")
        print("   - Đơn không tồn tại: 'Kiểm tra thông tin đơn hàng Order #3'")
        ...
        print(f"--- 🏁 DEMO CHẠY THỬ 1 TEST CASE MẪU ({tests[0].get('id', 'LT01')}: Tra cứu vận đơn) ---")
